...
const (
	...
	envClassifierWorkerConcurrency = "CLASSIFIER_WORKER_CONCURRENCY"

	defaultManagementListenAddr = "127.0.0.1:9091"

	defaultKafkaClientID = project.Name + "-classifier-worker"

	defaultClassifierWorkerConcurrency = 1

	maxClassifierWorkerConcurrency = 64
)
...
type classifierWorkerConfig struct {
	...
	classifierWorkerConcurrency int
}
...
func loadClassifierWorkerConfig(
	...
	classifierWorkerConcurrency := defaultClassifierWorkerConcurrency

	if rawConcurrency, ok := lookup(envClassifierWorkerConcurrency); ok {
		classifierWorkerConcurrency, err =
			parseClassifierWorkerConcurrency(rawConcurrency)
		if err != nil {
			return classifierWorkerConfig{}, err
		}
	}
	...
	return classifierWorkerConfig{
		...
		classifierWorkerConcurrency: classifierWorkerConcurrency,
	}, nil
}
...
func parseClassifierWorkerConcurrency(raw string) (int, error) {
	if strings.TrimSpace(raw) == "" {
		return defaultClassifierWorkerConcurrency, nil
	}
	...
	if err != nil {
		return 0, fmt.Errorf(
			...
			envClassifierWorkerConcurrency,
		)
	}

	if value < 1 || value > maxClassifierWorkerConcurrency {
		return 0, fmt.Errorf(
			"%s must be between %d and %d",
			envClassifierWorkerConcurrency,
			1,
			maxClassifierWorkerConcurrency,
		)
	}
