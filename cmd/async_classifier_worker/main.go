...
func run(logger *slog.Logger) error {
	...
	if err != nil {
		return fmt.Errorf("create Kafka producer client: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf(
			"create worker pool metrics: %w",
			err,
		)
	}

	workerPool, err :=
		application.NewClassificationWorkerPoolWithObserver(
			worker,
			config.classifierWorkerConcurrency,
			workerPoolMetrics,
		)
