...
func loadClassifierWorkerConfig(
	...
	if lookup == nil {
		return classifierWorkerConfig{}, errors.New("environment lookup must not be nil")
	}
	...
	consumerGroup, err := requiredTrimmedEnvironmentValue(lookup, envKafkaConsumerGroup)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	commandTopic, err := requiredTrimmedEnvironmentValue(lookup, envClassificationCommandTopic)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	resultTopic, err := requiredTrimmedEnvironmentValue(lookup, envClassificationResultTopic)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	commandDLQTopic, err := requiredTrimmedEnvironmentValue(lookup, envClassificationCommandDLQTopic)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	modelBundleVersion, err := requiredExactEnvironmentValue(lookup, envModelBundleVersion)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	manifestPath, err := requiredExactEnvironmentValue(lookup, envModelBundleManifestPath)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	tritonBaseURL, err := requiredExactEnvironmentValue(lookup, envTritonBaseURL)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	postgresDSN, err := requiredExactEnvironmentValue(lookup, envPostgresDSN)
	if err != nil {
		return classifierWorkerConfig{}, err
	}

	lightCurveBaseURL, err := requiredExactEnvironmentValue(lookup, envLightCurveBaseURL)
	if err != nil {
		return classifierWorkerConfig{}, err
	}
